...
# Usage: bash install-product-jieke-key.sh [existing-user] [public-key-file]
# Only old-key deletion requires confirmation. Success disables SSH passwords globally.
set -Eeuo pipefail
export PATH=/usr/sbin:/usr/bin:/sbin:/bin
umask 077
die() { printf '终止：%s\n' "$*" >&2; exit 1; }
[[ $# -le 2 ]] || die '用法：bash install-product-jieke-key.sh [用户名] [公钥文件]'
...
if [[ $# == 2 ]]; then
  ...
  printf '%s\n' 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIOj9rFzGhdA0/V4LaweKMTgINFFLmDElXXcMoRckAjZc' > "$tmp/input"
fi
...
def audit(work, target, home, uid, gid, config):
    ...
    canonical = ' '.join(current) + ' vps-product-jieke\n'
